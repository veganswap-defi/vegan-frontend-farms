...
const PageMeta = () => {
  ...
  const veganPriceUsdDisplay = veganPriceUsd.eq(0)
    ? ''
    : `$${veganPriceUsd.toNumber().toLocaleString(undefined, {
        minimumFractionDigits: 3,
        maximumFractionDigits: 3,
      })}`
  const pageMeta = customMeta[pathname] || {}
  const { title, description, image } = { ...DEFAULT_META, ...pageMeta }
  const pageTitle = veganPriceUsdDisplay ? [title, veganPriceUsdDisplay].join(' - ') : title
