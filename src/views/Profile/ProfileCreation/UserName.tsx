...
const minimumVeganToRegister = new BigNumber(REGISTER_COST).multipliedBy(DEFAULT_TOKEN_DECIMAL)
...
const UserName: React.FC = () => {
  ...
  const { teamId, selectedNft, userName, actions, minimumVeganRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumVeganRequired = useHasCakeBalance(minimumVeganToRegister)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumVeganRequired={minimumVeganRequired}
      allowance={allowance}
    />,
    false,
  )
    ...
    <>
      ...
      {!hasMinimumVeganRequired && (
        <Text color="failure" mt="16px">
          {t('A minimum of %num% VEGAN is required', { num: REGISTER_COST })}
        </Text>
      )}
    </>
  )
