...
import { useVegan, useBunnyFactory } from 'hooks/useContract'
...
const minimumVeganBalanceToMint = new BigNumber(MINT_COST).multipliedBy(DEFAULT_TOKEN_DECIMAL)

const Mint: React.FC = () => {
  const [variationId, setVariationId] = useState<Nft['variationId']>(null)
  const { actions, minimumVeganRequired, allowance } = useProfileCreation()

  const { account } = useWeb3React()
  const veganContract = useVegan()
  const bunnyFactoryContract = useBunnyFactory()
  const { t } = useTranslation()
  const hasMinimumVeganRequired = useHasCakeBalance(minimumVeganBalanceToMint)
  const {
    isApproving,
    isApproved,
    isConfirmed,
    ...
    onRequiresApproval: async () => {
      ...
      try {
        const response = await veganContract.methods.allowance(account, bunnyFactoryContract.options.address).call()
      ...
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" mb="24px" color="textSubtle">
            {t(`Cost: ${MINT_COST} VEGAN`, { num: MINT_COST })}
          </Text>
          {nfts.map((nft) => {
            ...
            return (
              <SelectionCard
                ...
                disabled={isApproving || isConfirming || isConfirmed || !hasMinimumVeganRequired}
              >
                <Text bold>{nft.name}</Text>
              </SelectionCard>
            )
          })}
          {!hasMinimumVeganRequired && (
            <Text color="failure" mb="16px">
              {t('A minimum of %num% VEGAN is required', { num: MINT_COST })}
            </Text>
          )}
          <ApproveConfirmButtons
            isApproveDisabled={variationId === null || isConfirmed || isConfirming || isApproved}
            isApproving={isApproving}
            isConfirmDisabled={!isApproved || isConfirmed || !hasMinimumVeganRequired}
