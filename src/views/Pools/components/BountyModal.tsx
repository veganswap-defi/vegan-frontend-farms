...
interface BountyModalProps {
  veganCallBountyToDisplay: string
  dollarCallBountyToDisplay: string
  totalPendingVeganRewards: BigNumber
  callFee: number
  onDismiss?: () => void
...
const BountyModal: React.FC<BountyModalProps> = ({
  veganCallBountyToDisplay,
  dollarCallBountyToDisplay,
  totalPendingVeganRewards,
  callFee,
  onDismiss,
  TooltipComponent,
}) => {
  ...
  const totalYieldToDisplay = getFullDisplayBalance(totalPendingVeganRewards, 18, 3)
  ...
  const handleConfirmClick = async () => {
    cakeVaultContract.methods
      ...
      .on('receipt', () => {
        toastSuccess(t('Bounty collected!'), t('VEGAN bounty has been sent to your wallet.'))
        setPendingTx(false)
        onDismiss()
      })
      .on('error', (error) => {
    ...
    <Modal title={t('Claim Bounty')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <Flex alignItems="flex-start" justifyContent="space-between">
        ...
        <Flex flexDirection="column">
          <Text bold>{veganCallBountyToDisplay} VEGAN</Text>
