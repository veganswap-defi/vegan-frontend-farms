...
const FeeSummary: React.FC<FeeSummaryProps> = ({ stakingTokenSymbol, lastDepositedTime, vaultFees, stakeAmount }) => {
  ...
  const feeInVegan = (parseFloat(stakeAmount) * (feeAsDecimal / 100)).toFixed(4)
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      ...
      <Text>
        {t(
          'Only applies within 3 days of staking. Unstaking after 3 days will not include a fee. Timer resets every time you stake new VEGAN in the pool.',
        )}
      </Text>
    </>,
    { placement: 'top-start' },
  ...
  return (
    <>
      <Flex mt="24px" alignItems="center" justifyContent="space-between">
        ...
        <Text fontSize="14px">
          {stakeAmount ? feeInVegan : '-'} {stakingTokenSymbol}
        </Text>
      </Flex>
