...
const CakeVaultCard: React.FC<{ pool: Pool; account: string }> = ({ pool, account }) => {
  ...
  const { totalVeganInVault, pricePerFullShare } = useGetVaultSharesInfo()
  ...
  return (
    <StyledCard isStaking={accountHasSharesStaked}>
      <StyledCardHeader isAutoVault earningTokenSymbol="VEGAN" stakingTokenSymbol="VEGAN" />
      <StyledCardBody isLoading={isLoading}>
        ...
        <Box mt="24px">
          <RecentCakeProfitRow
            account={account}
            veganAtLastUserAction={userInfo.veganAtLastUserAction}
      ...
      <CardFooter
        ...
        isAutoVault
        totalVeganInVault={totalVeganInVault}
      />
    </StyledCard>
  )
}
